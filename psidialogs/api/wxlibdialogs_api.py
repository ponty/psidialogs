...
def scrolledMessageDialog(
    parent=None, message="", title="", pos=wx.Point(-1, -1), size=(500, 300)
):
...
def messageDialog(
    parent=None, message="", title="Message box", aStyle=21, pos=wx.Point(-1, -1)
):
    return psidialogs.message(message=message, title=title)


def multipleChoiceDialog(
    parent=None,
    message="",
    title="",
    lst=[],
