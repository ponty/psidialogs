...
##[[[cog
##   import apigen
##   apigen.generateApi('easygui', 'easygui')
##]]]
def textbox(message='', title='', text='', codebox=0):
	...
	"""
    return common_api.text(message=message, title=title, text=text)
...
def choicebox(message='Pick something.', title='', choices=['program logic error - no choices specified']):
	...
	"""
    return common_api.choice(message=message, title=title, choices=choices)
def msgbox(message='Shall I continue?', title='', buttonMessage='OK'):
    """Original doc: Display a messagebox
	"""
    return common_api.message(message=message, title=title, ok=buttonMessage)
